...
- **Field-level encryption (CC6 — Confidentiality TSC):** confidential
  ...
  **Currently encrypted in fa-amort:**
    - `AiAssetSuggestion.inputText` (model-fed free-text)
    - `AiAssetSuggestion.outputJson` (Json mode — structured model
      response with embedded rationale)
    - `Party.displayName` (READ side — ledger-core writes; fa-amort
      reads via `vendor.displayName` on `/fixed-assets/[id]`)
