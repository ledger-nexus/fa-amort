...
8. **Mirror discipline: `db push` is BANNED.** `prisma/schema.prisma` declares a subset of the shared database, so `prisma db push` (or `migrate dev`) would execute the FULL diff — including DROP/ALTER statements against ledger-core-owned tables. There is no `db:push` npm script anymore; do not reintroduce it. The mirror is GENERATED from ledger-core's schema (currently main commit 9442667, 2026-07-16) and is FK-CLOSED — every foreign key on a mirrored table points at another mirrored table, so `npm run db:diff` produces ZERO statements against ledger-core-owned or fa-amort-owned tables. Schema changes to fa-amort-owned tables (`ai_asset_suggestion`) go through the reviewed-diff protocol: `npm run db:diff` → keep ONLY statements touching fa-amort-owned tables/enums → `npx prisma db execute --file <reviewed.sql>`. If the diff ever emits a statement against a mirrored table, the mirror has drifted — re-generate it from ledger-core (never hand-edit) before doing anything else. Enum-typed columns in ledger-core MUST stay enums here (a `String` mirror of an enum column is drift the diff will flag).
