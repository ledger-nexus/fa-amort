...
export const ENCRYPTED_COLUMNS: ReadonlyArray<{
  ...
  // indicator: a purchase description, invoice memo, news excerpt,
  // etc. The structured outputJson (Json column) routinely embeds
  // customer / vendor names + dollar amounts, but Json-column
  // encryption is a separate workstream — for now we encrypt the
  // String input which is the primary PII surface.
  //
  // Audited 2026-05-31 across all 5 repos: zero filter queries on
  // inputText. Only display reads in the AI audit panel.
  //
  // No relation paths to walk — AiAssetSuggestion is a leaf row
  ...
  // `vendor.displayName` for each asset. Without this registry
  // entry, that column would surface ciphertext. Mirrors the
  // recon-side rationale from commit 8a99009.
...
const RELATION_MAP: ReadonlyArray<{
  ...
  // fa-amort doesn't currently have any nested-write paths that
  // touch encrypted columns. FixedAsset.aiSuggestions is read-side
  // only (suggestions are written directly via
  // prisma.aiAssetSuggestion.create), and Party is read-only here.
];
