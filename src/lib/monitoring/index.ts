...
//
// Mirror of ledger-core's `src/lib/monitoring/index.ts` (PR #10 on
// ledger-core; closes the fa-amort half of deficiency #5 in the
// portfolio-wide v2.x risk register).
//
// Why this exists:
...
//      stack — which on fa-amort regularly embeds asset descriptions,
//      counterparty names, account codes. Going through this shim
//      runs `redactPii()` first.
...
//   } catch (e) {
//     captureError(e, {
//       context: "fa-amort/run-depreciation-stamp",
//       extra: { assetId, bookId, userId },
//     });
//     // Decide separately whether to swallow or re-throw — the shim
//     // does not control flow.
//   }
//
// This file closes the post-stamp `catch (e)` H1 finding from the
// 13th adversarial pass (fa-amort PR #20). The H1 fix today uses
...
/**
 * Capture a non-error event (e.g., "depreciation run skipped, no
 * periods due"). Use for security-relevant observations that aren't
